...
public class HeightMapGenerator : MonoBehaviour
{
    ...
    public float beachWidth = 20f; // Largeur de la plage
    public float beachHeight = 0.1f; // Hauteur de la plage
    ...
    float[] GenerateHeightMapCPU(int mapSize)
    {
        ...
        for (int y = 0; y < mapSize; y++)
        {
            for (int x = 0; x < mapSize; x++)
            {
                ...
                // Calcul de la distance depuis les bords
                float distX = Mathf.Min(x, mapSize - x);
                float distY = Mathf.Min(y, mapSize - y);
                float distFromEdge = Mathf.Min(distX, distY);

                // Hauteur constante pour la plage
                float beachFactor = Mathf.Clamp01(distFromEdge / beachWidth);
                float beachHeightValue = beachHeight * beachFactor;
                ...
                // Ajout de la hauteur de la plage
                noiseValue += beachHeightValue;
